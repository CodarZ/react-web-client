import type { ConfigExternal, OutputOptions } from 'orval';
...
/** 将服务配置数组转换为 Orval 配置对象 */
function buildConfig(): ConfigExternal {
  const config: Record<string, ReturnType<typeof createServiceConfig>> = {};
...
function createServiceConfig(config: ServiceConfig) {
  ...
  return {
    ...
    hooks: {
      afterAllFilesWrite: `node scripts/generate-endpoints-index.js && prettier --write ${basePath}`,
    },
  };
}
